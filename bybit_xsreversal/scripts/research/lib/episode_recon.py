...
def reconstruct_episodes(
    ...
    Returns:
      - episodes_df: episode_id, symbol, side (Long/Short), entry_ts, exit_ts, entry_vwap, exit_vwap,
                     ...
                     closed_pnl_mapped (bool), pnl_source (closed_pnl|fills|unmatched)
    ...
    if fills.empty:
        return (
            pd.DataFrame(columns=["episode_id", "symbol", "side", "entry_ts", "exit_ts", "entry_vwap", "exit_vwap", "fees_usd", "realized_pnl_usd", "hold_hours", "num_fills", "max_abs_position", "closed_pnl_mapped", "pnl_source"]),
        ...
        for idx in range(len(grp)):
            ...
            if episode_id is None:
                if abs(position_after) > 1e-12:
                    ...
                    exit_sum_pq = 0.0
                    episode_fees = 0.0
                    episode_pnl_from_fills = float(pnl) if pnl is not None else 0.0
                ...
                episode_fees += fee
                if pnl is not None:
                    episode_pnl_from_fills += float(pnl)
                if oid:
